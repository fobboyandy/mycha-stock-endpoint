...
for location_name, groupid in groups.items():
    
    if groupid not in inventory_by_group:
        inventory_by_group[groupid] = {}
        
    inventory_by_group[groupid][location_name] = remaining_inventory_by_location[location_name]
        
#organized as group:location:item name, count
consolidated_inventory = {}
#insert item name
for group in inventory_by_group:

    if group not in consolidated_inventory:
        consolidated_inventory[group] = {}

    for location in inventory_by_group[group]:

        if location not in consolidated_inventory[group]:
            consolidated_inventory[group][location] = []
            ...
            for row in range(6):
                ...
                for col in range(7):
                    ...
                    count = inventory_by_group[group][location][row][col] 
                    row_items.append((name, count))
                    
                consolidated_inventory[group][location].append(row_items)
                
# print(json.dumps({"hi": "hi"}))
print(json.dumps(consolidated_inventory["LA"]["Santa Monica College"]))

# sys.stdout = io.TextIOWrapper(io.BufferedWriter(consolidated_inventory, 100000))
# print(sys.stdout)
